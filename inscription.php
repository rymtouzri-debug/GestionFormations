...
<div class="form-box">
    ...
    <form method="POST" action="traitement.php">
        ...
        <input type="text" name="nom" value="<?= htmlspecialchars($old_post['nom'] ?? '') ?>" placeholder="Ex : Ben Ali" required>

        <label>Prénom <span class="required">*</span></label>
        <input type="text" name="prenom" value="<?= htmlspecialchars($old_post['prenom'] ?? '') ?>" placeholder="Ex : Asma" required>

        <label>Adresse Email <span class="required">*</span></label>
        <input type="email" name="email" value="<?= htmlspecialchars($old_post['email'] ?? '') ?>" placeholder="exemple@email.com" required>
        ...
        <select name="formation_id" required>
            ...
            <?php foreach ($formations as $f): ?>
                <option value="<?= $f['id'] ?>"
                    <?= (($old_post['formation_id'] ?? $formation_preselect) == $f['id']) ? 'selected' : '' ?>>
